...
type IntegrationTests(factory: WebApplicationFactory<Program.Startup> ) =
    ...
    do
        System.IO.File.Delete(sprintf "%s.json" defaultState.Game.Id)
    ...
    let ``GET /name should say my name`` () =
        task {
            ...
            let data = JsonSerializer.Deserialize<Handlers.NameResponse>(json, options)
    ...
    let ``Should go to neutral``() =
        task {
            let client = factory.CreateClient()
            let requestBody = JsonSerializer.Serialize(defaultState, options)
            let! response = client.PostAsync("/move", new StringContent(requestBody))
            response.EnsureSuccessStatusCode() |> ignore

            let! json = response.Content.ReadAsStringAsync()
            let data = JsonSerializer.Deserialize<Handlers.MoveResponse>(json, options)
            [data.Move]  |> should be (subsetOf [Types.Move.Up; Types.Move.Left])         
    ...
    let ``Should shoot neutral``() =
        task {
            let client = factory.CreateClient()
            let state = { defaultState with Player = { defaultState.Player with Position = {Y=1;X=3}}}
            ...
            let! json = response.Content.ReadAsStringAsync()
            let data = JsonSerializer.Deserialize<Handlers.MoveResponse>(json, options)
            [data.Move]  |> should be (subsetOf [Types.Move.``Fire-left``])         
        }
    ...
    let ``Should not shoot neutral cause of wall``() =
        task {
            let client = factory.CreateClient()
            let walls = {Y=1;X=2} :: defaultState.Board.Walls
            let state = { defaultState with
                              Player = { defaultState.Player with Position = {Y=1;X=3}}
                              Board = { defaultState.Board with Walls = walls }
                              }
            let requestBody = JsonSerializer.Serialize(state, options)
            let! response = client.PostAsync("/move", new StringContent(requestBody))
            response.EnsureSuccessStatusCode() |> ignore
            ...
            let! json = response.Content.ReadAsStringAsync()
            let data = JsonSerializer.Deserialize<Handlers.MoveResponse>(json, options)
            [data.Move]  |> should be (subsetOf [Types.Move.Down])         
        }
    ...
    let ``Should shoot agressif first``() =
        task {
            let client = factory.CreateClient()
            let enemies = {Y=3;X=3; Neutral = false} ::  defaultState.Enemies
            let state = { defaultState with
                          Player = { defaultState.Player with Position = {Y=1;X=3}}
                          Enemies = enemies
                                                        }
            ...
            let! json = response.Content.ReadAsStringAsync()
            let data = JsonSerializer.Deserialize<Handlers.MoveResponse>(json, options)
            [data.Move]  |> should be (subsetOf [Types.Move.``Fire-down``])         
        }
    ...
    let ``Should get neutral before shooting``() =
        task {
            let client = factory.CreateClient()
            let enemies = {Y=3;X=2; Neutral = false} ::  defaultState.Enemies
            let state = { defaultState with
                          Player = { defaultState.Player with Position = {Y=1;X=2}}
                          Enemies = enemies
                                                        }
            let requestBody = JsonSerializer.Serialize(state, options)
            let! response = client.PostAsync("/move", new StringContent(requestBody))
            ...
            let! json = response.Content.ReadAsStringAsync()
            let data = JsonSerializer.Deserialize<Handlers.MoveResponse>(json, options)
            [data.Move]  |> should be (subsetOf [Types.Move.Left])         
        }
    ...
    let ``Should get neutral before shooting again``() =
        task {
            let client = factory.CreateClient()
            let enemies = [{Y=3;X=1; Neutral = false}; {Y=1;X=1; Neutral = true}]
            let state = { defaultState with
                          Player = { defaultState.Player with Position = {Y=1;X=2}}
                          Enemies = enemies
                                                        }
    ...
    let ``Should get get away from enemy if cannot shoot``() =
        task {
            ...
            let state = { defaultState with
                            Enemies = enemies
                            Player = {defaultState.Player with Fire = false }
                                                        }
            let requestBody = JsonSerializer.Serialize(state, options)
            let! response = client.PostAsync("/move", new StringContent(requestBody))
            response.EnsureSuccessStatusCode() |> ignore
            ...
            let! json = response.Content.ReadAsStringAsync()
            let data = JsonSerializer.Deserialize<Handlers.MoveResponse>(json, options)
            [data.Move]  |> should be (subsetOf [Types.Move.Up])         
        }
